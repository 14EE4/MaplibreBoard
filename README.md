...
- **악성 사용자 IP 차단 (IP 밴) 및 DDoS 방지:** (NEW!)
  - *글 기반 IP 차단*: 어드민 게시글 목록에서 작성자의 IP를 클릭 한 번으로 간편하게 차단할 수 있습니다. 밴 처리 시 원본 게시글 내용이 자동으로 차단 사유에 첨부되며 추가적인 관리자 사유 작성이 가능합니다.
  - *직접 IP 입력 차단*: 어드민 대시보드의 'IP Bans' 탭에서 특정 IP 주소와 차단 사유를 직접 기입하여 밴을 등록하고 해제(Unban)할 수 있습니다.
  - *입구 차단 (DDoS 보호)*: 차단된 IP가 일반 사용자용 API(글 작성/수정/삭제, 게시판 생성, 이미지 업로드, 게시글 권한 검증 등)에 요청을 시도할 경우, 데이터베이스의 유니크 인덱스를 조회하여 차단 여부를 검증하고, 무거운 쿼리나 비즈니스 로직을 일절 타지 않고 즉시 `403 Forbidden` 상태코드와 함께 밴 상태(`{ banned: true, reason: '...' }`)를 반환합니다. 이는 쿼리 병목이나 DB 커넥션 고갈 등을 유발하는 조회/작성 공격(DDoS)을 가볍게 방어해 줍니다.
  - *전역 fetch 인터셉터 알림*: 클라이언트 전역에서 403 밴 응답을 감지하여 차단 사유와 함께 경고 알림(`alert`)을 팝업으로 표시합니다.
  - *어드민 안전 접속 보장*: 실수로 관리자 본인의 IP를 차단하더라도, 어드민 기능 관련 API(`/api/admin/*`)와 어드민 로그인 화면에는 IP 차단 미들웨어를 적용하지 않아 언제든 어드민 페이지에 접속해 직접 차단을 해제할 수 있도록 안전 장치가 구현되었습니다.
...
### 🔗 URL 주소 하이퍼링크 자동 감지 및 변환 (NEW)
- **자동 감지 및 클릭 가능한 링크 전환:** 게시글 본문(post.content) 렌더링 시 URL 패턴(`https?://...`)을 정규식으로 실시간 자동 감지하여 클릭 가능한 하이퍼링크(`<a href="..." target="_blank" rel="noopener noreferrer">`)로 변환합니다.
- **안전한 React 구조적 렌더링 (XSS 방지):** 악의적인 스크립트 실행 공격(XSS)을 원천 방지하기 위해 `dangerouslySetInnerHTML`을 절대 사용하지 않습니다. 글 본문을 정규식 기준으로 분할(`split`)하여 텍스트 노드와 안전한 React `<a>` 컴포넌트로 개별 매핑합니다.
- **지도 유지 및 새 탭 열기 (target="_blank"):** 사용자가 보던 지도 화면 및 기존 세션 상태가 끊기지 않도록 모든 외부 링크는 새 탭으로 강제 연동합니다.
- **글 본문 줄바꿈 및 인용 기능과의 상호 호환:** 줄바꿈(`<br />`) 및 기존 `>>XX` 번호 인용 텍스트 감지 로직과 완전히 어우러져 작동하도록 통합 렌더링 파이프라인([PostContent.js](file:///home/pyeongju/workspace/MaplibreBoard/components/PostContent.js))을 통해 제공합니다.
