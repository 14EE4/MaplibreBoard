...
- **이미지 업로드 및 Lightbox:** 게시물 작성 시 이미지를 첨부(Base64 업로드)하여 로컬 디렉토리에 저장하고, Lightbox를 통해 모달 형식의 확대 보기가 가능합니다. (MIME 확장자 allow-list 검증을 통해 허용되지 않는 확장자 및 SVG 파일 기반의 Stored XSS 보안 취약점 차단 완료)
