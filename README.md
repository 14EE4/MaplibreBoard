...
### 🎯 그리드 보드 및 게시글
- **격자 보드 이동 및 수동 생성:** 지도 클릭 시 해당 격자 보드 페이지(`/board?grid_x=XX&grid_y=YY`)로 즉시 이동하며, 보드가 등록되어 있지 않은 경우 사이드바를 통해 기본 이름(격자 좌표) 또는 **사용자가 직접 입력한 커스텀 이름(Custom Name)**으로 지정하여 생성할 수 있도록 일원화했습니다. (이동 후 즉각적인 생성 및 화면 리로드 없는 실시간 갱신 지원)
...
### 🌐 글로벌 UI 다국어화 및 이모지(Emoji) 적용 (NEW)
- **글로벌 이모지 레이블:** 텍스트 기반 UI의 다국어 대응 리소스를 최소화하고 직관성을 높이기 위해 주요 조작 버튼을 글로벌 공통 이모지로 대체했습니다.
  - 지도 옵션: `격자 크기` ➡️ `🌐`, `지도 변경` ➡️ `🗺️`
  - 게시글 액션: `글쓰기` ➡️ `✏️`, `수정` ➡️ `🛠️`, `삭제` ➡️ `🗑️`
  - 어드민 인증: `로그인` ➡️ `🔒`
  - 각 이모지 버튼에 마우스 오버 시 직관적인 설명을 제공하는 툴팁(`title` 속성)을 보완했습니다.
- **남은 UI 및 오류 메시지 영어화:** 서비스 전반에 남은 한국어 텍스트 레이블, 알림 팝업(Alert/Toast), API 에러 응답 및 IP 대역 정보(사설 IP ➡️ `Private IP`, 일반 공인 IP ➡️ `Public IP`, 통신사 모바일 대역 등)를 영문으로 완전히 번역 및 통일하여 글로벌 인터페이스 접근성을 제공합니다.
...
- **일반 보드 생성:** 격자 좌표(Grid Coordinates)에 연결되지 않는 독립적인 일반 보드를 생성하여 바로 오픈할 수 있는 기능 제공


### 📊 기기 및 위치 정보 분석 (GeoIP & UA Parser) (NEW)
- **접속 IP 기반 대략적인 위치 분석 (GeoIP):** 외부 API 의존성 없이 `fast-geoip` 라이브러리를 통해 로컬에서 클라이언트 IP 기준의 국가 및 도시 정보(`location`)를 실시간 매핑하여 데이터베이스에 함께 기록합니다. (로컬/사설망 대역은 `Local`로 자동 분류하며, 무료 MaxMind GeoLite2 로컬 DB의 한계로 인해 일부 글로벌 호스팅/VPN 대역 IP의 경우 물리적 노드 위치가 아닌 회사 본사 등록국가(예: US 등)로 수집되는 위치 편차가 존재할 수 있습니다.)
...
- [x] 0,0 좌표 보정: 특정 좌표 클릭 시 비정상 이동 문제 디버깅
