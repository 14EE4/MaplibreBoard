...
export default async function handler(req, res) {
  ...
  if (auth !== ADMIN_PASSWORD) {
    ...
    console.log(`[${timestamp}] [API LOG] [401 Unauthorized] 어드민 전체 게시글 목록 조회 실패 - 인증 실패`);
    return res.status(401).json({ error: 'Unauthorized' });
  }

  try {
    const timestamp = new Date().toISOString();
    console.log(`[${timestamp}] [API LOG] 어드민 전체 게시글 목록 조회 시도`);
    ...
    console.log(`[${successTimestamp}] [API LOG] 어드민 전체 게시글 목록 조회 성공 - 총 ${posts.length}건`);
